...
const PandaBear = () => {
  ...
  const increasePandaBears = useBearStore(state => state.increasePandaBear);
  return (
    <WhiteCard centered>
      <h2>Osos Negros</h2>
      <div className="flex flex-col md:flex-row">
...
export const BearDisplay = () => {
  ...
  // useShallow compara los objetos y evita rerenders
  const bears = useBearStore(useShallow(state => state.bears))
  const doNothing = useBearStore(state => state.doNothing)
  const addBear = useBearStore(state => state.addBear)
  const clearBears = useBearStore(state => state.crearBears)
